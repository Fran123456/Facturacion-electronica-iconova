...
use App\Models\MH\MHCondicionOperacion;
use App\Models\MH\MHPlazo;
...
class PdfController extends Controller
{
    ...
    public function documentData($CodGeneracion)
    {
        ...
        if ($tipo_documento == '11') {
            ...
            $receptor_direccion = isset($JsonDTE['receptor']['direccion']['complemento']) ? $JsonDTE['receptor']['direccion']['complemento'] : null;
        }
        // DATOS DEL RECEPTOR (MUNICIPIO, DEPARTAMENTO, DIRECCION)
        ...
        foreach ($aDetalle as $key => $row) {
            ...
            $total_letras = Generator::generateStringFromNumber($totalPagar);
        ...
        $cfoot2 = '<tr style="border-top: solid 1px;">
                 <td colspan="5" style="text-align: right;">Suma de operaciones</td>
                 <td></td>
        ...
        if ($tipoDte != '01') {
            $cfoot3 = '<tr><td colspan="5" style="text-align: right;">IVA</td>
                 <td></td>
                 <td style="text-align: right;">' . number_format($total_totaliva, 2) . '</td></tr>';
        }
        $footRenta = null;
        if ($tipoDte == '14') {
            $footRenta = '<tr><td colspan="5" style="text-align: right;">Renta</td>
                 <td></td>
                 <td style="text-align: right;">' . number_format($renta, 2) . '</td></tr>';
        }

        $cfoot4 = '<tr>
                 <td colspan="5" style="text-align: right;">Suma de ventas no sujetas</td>
                 <td></td>
                 <td style="text-align: right;">' . number_format($total_descuentonosujetas, 2) . '</td></tr>';

        $cfoot5 = '<tr>
                 <td colspan="5" style="text-align: right;">Suma de ventas exentas</td>
                 <td></td>
        ...
        $cfoot7 = '<tr>
                 <td colspan="5" style="text-align: right;">IVA retenido</td>
                 <td></td>
        ...
        $cfoot8 = '<tr style="border-top: solid 1px;background:black;color: white;">
                 <td colspan="5" style="text-align: right;"><b>total a pagar o saldo a favor</b></td>
                 <td></td>
                 <td style="text-align: right;"><b>' . number_format($totalPagar, 2) . '</b></td></tr>';

        $cfoot9 = '<tr style="border-top: solid 1px;">
                 <td colspan="5" style="text-align: left;"><b>Valor en letras:' . $total_letras . ' USD</b></td>
                 <td></td><td></td></tr>';
